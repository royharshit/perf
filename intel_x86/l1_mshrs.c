...
#define L1D_PEND_MISS_PENDING_EVENT 0x48
#define L1D_PEND_MISS_PENDING_UMASK 0x01
...
void post_processing(int num_events, long long int (*pc)[20]) {
    ...
    //L1 Mshrs 
    for (uint32_t i = 0; i < 10; i++) {
	printf("L1 Mshr (Demand Requests outstanding Non-Prefetch) H%d = %f\n", i, (double)pc[2][i]/pc[0][i]);
    } 

    //L1 Mshrs 
    for (uint32_t i = 10; i < 20; i++) {
	printf("L1 Mshr (Demand Requests outstanding Prefetch) H%d = %f\n", i-10, (double)pc[2][i]/pc[0][i]);
    } 
}
...
int main() {
    ...
    long int event_codes[] = {PERF_COUNT_HW_CPU_CYCLES, L1D_PEND_MISS_FB_FULL_EVENT | L1D_PEND_MISS_FB_FULL_UMASK << 8, L1D_PEND_MISS_PENDING_EVENT | L1D_PEND_MISS_PENDING_UMASK << 8}; // Example event codes
    const char *event_names[] = {"Total cyles", "Fill Buffer Full", "L1 Outstanding due to demand requests"};
    long int event_types[] = {PERF_TYPE_HARDWARE, PERF_TYPE_RAW, PERF_TYPE_RAW};
